...
function go(tab){
  ...
  if(!social){setTimeout(()=>go(tab),500);return}
  const btn=social.querySelector(`[data-tab="${tab}"]`);
  if(btn)btn.click();
  social.scrollIntoView({behavior:'smooth',block:'start'});
}
document.addEventListener('click',e=>{const b=e.target.closest('[data-social-go]');if(b)go(b.dataset.socialGo)});
